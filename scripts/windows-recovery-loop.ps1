...
if ($IntervalSeconds -le 0) {
  if ($env:ARL_RECOVERY_INTERVAL_SECONDS) {
    $IntervalSeconds = [int]$env:ARL_RECOVERY_INTERVAL_SECONDS
  } else {
    $IntervalSeconds = 30
  }
}
...
Write-Host "[ARL] recovery loop started"
...
while ($true) {
  try {
    & $venvPython -m arl.cli recovery
    if ($LASTEXITCODE -ne 0) {
      Write-Warning "[ARL][warn] recovery run failed (exit $LASTEXITCODE); continue after sleep"
    }
  } catch {
    Write-Warning "[ARL][warn] recovery threw: $($_.Exception.Message); continue after sleep"
  }
