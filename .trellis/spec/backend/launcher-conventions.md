...
> Conventions for the PowerShell launcher scripts under `scripts/`
> that bootstrap the agent / orchestrator / recorder / postprocess / recovery
> loops on Windows. Not for `src/arl/...` business logic.
...
The repo has five PowerShell launcher scripts under `scripts/`:
...
- `scripts/windows-postprocess-loop.ps1` - post-live processing loop (drives single-pass `arl.cli postprocess --once` invocations + supervises restart on crash)
- `scripts/windows-recovery-loop.ps1` - recovery dispatch loop (drives single-pass `arl.cli recovery` invocations + supervises restart on crash)
- `scripts/windows-supervisor.ps1` - optional parent supervisor that starts the five launchers hidden and restarts any exited child
- `scripts/windows-autostart.ps1` - optional opt-in Scheduled Task manager for logon autostart
...
all five. This document captures the conventions and the pitfalls already hit.
...
### Windows postprocess interval

`scripts/windows-postprocess-loop.ps1` MUST drive the single-pass
`arl postprocess --once` command from a PowerShell loop. The interval is
resolved in this order:

1. `-IntervalSeconds`
2. `ARL_POSTPROCESS_INTERVAL_SECONDS`
3. default `30`

The postprocess launcher intentionally does not run `arl cookie-health`; it
does not directly touch platform auth. It should catch native command failures,
emit one warning, sleep, and continue, matching the recorder launcher
supervision shape.

### Windows recovery interval

`scripts/windows-recovery-loop.ps1` MUST drive the single-pass `arl recovery`
command from a PowerShell loop. The interval is resolved in this order:

1. `-IntervalSeconds`
2. `ARL_RECOVERY_INTERVAL_SECONDS`
3. default `30`

The recovery loop dispatches manual recovery actions generated by the recorder
so `arl status` does not remain stuck on `recovery_undispatched_actions` during
unattended runs. It does not mark actions resolved or failed; those transitions
remain explicit operator commands.

### Optional Windows autostart

Autostart MUST be opt-in. A checkout, install, or normal launcher run must not
register a Scheduled Task by itself.

Use `scripts/windows-autostart.ps1`:

| Action | Behavior |
|---|---|
| `Status` (default) | Read-only; reports whether the task exists |
| `Install` | Registers a Scheduled Task that runs `windows-supervisor.ps1`; default trigger is `AtLogOn` |
| `Uninstall` | Removes the Scheduled Task if present |

The supervisor task runs hidden and writes child launcher logs under
`data/tmp/launcher-logs/`. `windows-supervisor.ps1` owns process-level restart
for the five launcher windows; Task Scheduler only starts the supervisor at
logon and applies a small restart policy if the supervisor itself exits.

Do not use `AtStartup` by default; it often requires elevated privileges and
runs before a normal interactive user profile is ready. `AtLogOn` is the
production baseline for this local Windows workstation deployment. Operators
who explicitly need machine-start behavior may pass `-TriggerMode AtStartup`.
...
**Symptom**: Operator runs the PowerShell launcher windows and finds the
...
change to its peers, because the scripts are not generated from a
shared template.

**Fix**: Update all affected launchers in the same task. Add a comment on
...
default value, sentinel name, and trigger condition match across all launchers.
...
- A unified `windows-fast.ps1` master entrypoint that wraps all loops.
