...
- CLI `subtitles` generation contract:
  ...
  - successful ASR entries are normalized to Simplified Chinese/term fixes first, then display-smoothed before SRT persistence. Display smoothing is controlled by `ARL_ASR_DISPLAY_SMOOTHING_ENABLED`, `ARL_ASR_DISPLAY_MIN_DURATION_SECONDS`, `ARL_ASR_DISPLAY_TRAILING_HOLD_SECONDS`, and `ARL_ASR_DISPLAY_MAX_GAP_FILL_SECONDS`.
  - display smoothing must keep cue starts at the ASR word/segment start, extend short cues up to the configured minimum/hold duration, fill only gaps to the next cue that are `<= display_max_gap_fill_seconds`, and clamp cue ends to the match boundary duration. It must not affect placeholder SRT generation. At most two cues may be on screen at once: cue i's display end is clamped to cue i+2's start (human review 2026-07-10). Defaults were retuned the same day (min duration 2.5s, gap fill 1.5s) because 8s gap fill kept lines on screen long after speech ended; subtitle-active ratio lands lower than the 2026-07-09 0.8+ reference.
  - unit tests for subtitle generation must keep exact raw-timing assertions possible by disabling smoothing in the fixture, and must include a dedicated regression asserting short-cue extension plus small-gap fill.
...
- Copywriter generation contract:
  ...
  - publishing package JSON must include `summary`, `cover_lines`, `evidence`, title candidates, recommended title, tags, transcript excerpt, source subtitle/export/recording paths, optional `streamer_name`, optional `cover_path`, `cover_candidates`, optional `published_package_dir`, optional published video/cover/metadata paths, `status`, and `created_at`. `cover_candidates` is additive and defaults to an empty list for legacy rows.
  ...
  - when `cover_path` exists, copywriter copies the top-ranked generated cover image inside the package directory as `cover.<source suffix>` and records it as `published_cover_path`. The rank-1 `cover_candidates` entry reuses that same `cover.jpg` path; only rank 2 and later publish as `cover-02.jpg`, `cover-03.jpg`, etc. A stale `cover-01.*` marks the package incomplete and is removed during repair. This gives operators a named folder containing the video plus optional additional cover choices without duplicating the default cover.
  ...
  - optional cover rendering may use ffmpeg to extract source frames and Pillow to render `1920x1080` JPEG candidates under `data/processed/<session_id>/match-<idx>-cover-01.jpg`, `cover-02.jpg`, and `cover-03.jpg`; it should prefer the raw `RecordingAsset` as the frame source and use source-timeline offsets there, but when the recording asset/file is unavailable and `source_export_path` exists it must fall back to a single exported-MP4 candidate at export time `0.0`. Candidate frame selection may use KDA cues, valid edit-plan teaser/high-signal windows, highlight windows, scene class, sharpness/brightness, and bottom-left chat-region activity. Rendered cover text must consume `cover_lines` and use stacked yellow headline lines with a heavy black stroke inside safe margins. Missing recording/export files, missing ffmpeg, missing Pillow/cv2, or rendering errors must skip or degrade cover output while still writing publishing metadata.
