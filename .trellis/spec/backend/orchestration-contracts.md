...
- CLI `subtitles` generation contract:
  ...
  - display smoothing must keep cue starts at the ASR word/segment start, extend short cues up to the configured minimum/hold duration, fill only gaps to the next cue that are `<= display_max_gap_fill_seconds`, and clamp cue ends to the match boundary duration. It must not affect placeholder SRT generation.
