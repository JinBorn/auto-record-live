...
- Copywriter generation contract:
  ...
  - when `cover_path` exists, copywriter copies the top-ranked generated cover image inside the package directory as `cover.<source suffix>` and records it as `published_cover_path`. If `cover_candidates` exist, copywriter also copies each candidate as `cover-01.jpg`, `cover-02.jpg`, etc. and records each `published_path`. This gives operators a named folder containing the video plus cover choices without needing to inspect `data/processed`, while keeping `cover.jpg` as the legacy default upload cover.
